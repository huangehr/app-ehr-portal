...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
		var that = this, DOM,
			icon = config.icon,
			iconBg = icon && (_isIE6 ? {png: '../widget/artDialog/4.1.7/images/' + icon + '.png'}
			: {backgroundImage: 'url(\'' + '../widget/artDialog/4.1.7/images/' + icon + '.png\')'});
		var artIcon='';
	...
	close: function () {
		...
		var that = this,
			...
			closeAnimatedTime = that.config.closeAnimatedTime;

		function closeBefroe(){
			that.time();
		...
		if(closeAnimatedTime != 0){
			wrap.animate({opacity: 0}, closeAnimatedTime,'linear',function(){
				closeBefroe();
				wrap.remove();
			});
		}else{
	...
	time: function (second) {
		var that = this,
			...
			timer = that._timer,
			wrap = that.DOM.wrap,
			mouseenterOffTime = that.config.mouseenterOffTime;

		timer && clearTimeout(timer);
		if(mouseenterOffTime){
		...
		if (second) {
			function timerFun (){
				that._timer = setTimeout(function(){
					if(mouseenterOffTime && that.offTimeB){
						setTimeout(function(){
							timerFun();
						},1000 * second);
					}else{
	...
	lock: function () {
		...
		if(config.quickClose){
			lockMaskWrap.bind('click',function(){
				that._click(that.config.cancelVal);
			});
		}
...
artDialog.defaults = {
	...
	artIcon: '',				// 标题旁边的图标,
	tips: false,				// 提示框
	closeAnimatedTime:0,		// 对话框关闭时的动画时间
	mouseenterOffTime:false		// 自动关闭开始时鼠标移入取消自闭，移出再次开启
};
