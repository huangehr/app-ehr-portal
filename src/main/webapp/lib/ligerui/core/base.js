...
(function ($) //jQuery ligerUI 1.3.2
{
    ...
    liger.win = {
        ...
        mask: function (win)
        {
            ...
            // TODO [代码片段001][修复dialog在load方式下关闭对话框后遮罩仍显示的问题][yezehua]
            //if (!this.windowMask)
            //{
            ...
            //setHeight();
            //this.masking = true;
            // TODO [代码片段001][修复dialog在load方式下关闭对话框后遮罩仍显示的问题][yezehua] 结束

            if (!win.windowMask)
            {
                win.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
                $(window).bind('resize.ligeruiwin', setHeight);
                $(window).bind('scroll', setHeight);
            }
            win.windowMask.show();
            ...
            // TODO [代码片段005][修复dialog的遮罩在页面出现滚动条后不能随其滚动遮盖的问题][yezehua]
            this.windowMask = win.windowMask;
            // TODO [代码片段005][修复dialog的遮罩在页面出现滚动条后不能随其滚动遮盖的问题][yezehua]
        },
        ...
        unmask: function (win)
        {
            ...
            for (var i = 0, l = jwins.length; i < l; i++)
            {
                ...
                // TODO [代码片段003][修复dialog多层弹出框都关闭时，还存在遮罩的问题][yezehua]
                //如果存在其他模态窗口，那么不会取消遮罩
                //if (modal) return;
                // TODO [代码片段003][修复dialog多层弹出框都关闭时，还存在遮罩的问题][yezehua] 结束
            }
            // TODO [代码片段002][修复dialog在load方式下关闭对话框后遮罩仍显示的问题][yezehua]
            //if (this.windowMask)
            //    this.windowMask.hide();
            //this.masking = false;
            // TODO [代码片段002][修复dialog在load方式下关闭对话框后遮罩仍显示的问题][yezehua] 结束
            if (win.windowMask)
                win.windowMask.hide();
            win.masking = false;
        },
        ...
        // TODO [代码片段004][修复多层弹出框z-index引起的遮罩层与弹出框显示层次问题][yezehua]
        //前端显示
        ...
        setFront: function (win)
        {
            ...
            for (var i in wins)
            {
                ...
                if (w == win)
                {
                    var maxZ = getMaxZindex(win);
                    w.windowMask[0].style['z-index'] = maxZ + 100;
                    $(w.element).css("z-index",  maxZ + 200);
                    this.activeTask(w);
                }
                else
                {
                    var zindex = parseInt(w.windowMask[0].style['z-index']) || 9000;
                    $(w.element).css("z-index", zindex + 100);
                }
            }

        }
        // TODO [代码片段004][修复多层弹出框z-index引起的遮罩层与弹出框显示层次问题][yezehua] 结束
    };
    ...
    function getMaxZindex (win) {
        ...
        for (var i in wins)
        {
            var w = wins[i];
            if(w != win)
                zindexs.push(w.element.style['z-index']);
        }
        if(!zindexs.length)
            return 9000;
