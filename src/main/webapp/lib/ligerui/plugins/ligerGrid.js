...
(function ($)
{
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                          <div class='l-grid-header-inner'><table class='l-grid-header-table' cellpadding='0' cellspacing='0'><tbody></tbody></table></div>");
            gridhtmlarr.push("                      </div>");
            gridhtmlarr.push("                      <div class='m-custom-scroll'><div  class='m-grid-body'><div class='l-grid-body l-grid-body2 l-scroll'>");
        ...
        */
        ...
        {
            var g = this, p = this.options;
            //if (column._width) return column._width; // TODO: [代码片段001] [修正列宽自适应窗口变化] [yezehua]
        ...
        _getCellHtml: function (rowdata, column)
        {
            ...
            // TODO [代码片段004][增加鼠标悬置在单元格时内容title提示][yezehua]
            var title = content;
            var reg = new RegExp('^<([^>]+)[^>]*>');

            if(/^<select/.test(title)) {
                title = '';
            } else if(reg.test(title)) {
                title = $(title).text();
            }
            var _content ='" title="'+title+'"><div>' + content + '</div></div>';
            if(column.align=="left")
            {
                _content ='" title="'+title+'"><div style="margin-left: 8px;">' + content + '</div></div>';
            }
            htmlarr.push(_content);
            // htmlarr.push('">' + content + '</div>');
            // TODO [代码片段004][增加鼠标悬置在单元格时内容title提示][yezehua] 结束
            return htmlarr.join('');
        },
        ...
        _getTreeCellHtml: function (oldContent, rowdata)
        {
            ...
            if (isExtend && isParent && typeof(rowdata.children)=='object' &&rowdata.children.length>0)
                content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-open'></div>";
            else if (isParent && typeof(rowdata.children)=='object'&& rowdata.children.length>0)
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                // TODO [代码片段003][修复mcustomscroll滚动后，编辑框偏移问题][yezehua]
                if(g.mScroller) {
                    top -= g.f.gridbody[0].scrollTop;
                }
                // TODO [代码片段003][修复mcustomscroll滚动后，编辑框偏移问题][yezehua] 结束
                jcell.html("");
                g.setCellEditing(rowdata, column, true);
        ...
        _setEvent: function ()
        {
            ...
            // TODO [代码片段002][增加跳转分页输入框回车事件][yezehua]
            //工具条 - 切换当前页事件
            $('span.pcontrol :text', g.toolbar).keydown(function (e)
            {
                if(e && e.keyCode==13) g.changePage('input');
            });
            // TODO [代码片段002][增加跳转分页输入框回车事件][yezehua] 结束
