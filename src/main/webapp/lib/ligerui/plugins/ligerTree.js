    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        update: function (domnode, newnodedata)
        {
            ...
            for (var attr in newnodedata)
            {
                ...
                {
                    $("> .l-body > span", domnode).text(newnodedata[attr]).css({
                        "line-height": "22px"
                    });
                }
            }
        ...
        getDataByID: function (id)
        {
            ...
            function find(items)
            {
                for (var i = 0; i < items.length; i++)
                {
                    ...
                    // TODO [代码片段002][修复getDataByID方法只能查找第一子节点下的节点BUG][yezehua]
                    // if (dataItem.children && dataItem.children.length) return find(dataItem.children);
                    if (dataItem.children && dataItem.children.length) {
                        var item = null;
                        item = find(dataItem.children);
                        if(item) return item;
                    }
                    // TODO [代码片段002][修复getDataByID方法只能查找第一子节点下的节点BUG][yezehua] 结束
                }
                return null;
            }
